 ...
 */
public class conflict extends Tffst {
  ...
  public conflict() {
	...
	TfI hp = new SimpleTf("h_prof");
	TfI mhp = new SimpleTf("mh_prof");
	TfI see = new SimpleTf("see");
	TfI pat = new SimpleTf("pat");
	TfI write = new SimpleTf("write");
	TfI rec = new SimpleTf("record");
	...
	s0.addOutTran(new Transition(hp, hp, s1, 1));
	s1.addOutTran(new Transition(see, see, s2, 1));
	
	Transition tran = new Transition(pat, pat, s3, 1);
	tran.getLabelOut().addRetTFString(hp).addRetTFString(write).addRetTFString(rec);
	s2.addOutTran(tran);
	
	s0.addOutTran(new Transition(mhp, mhp, s4, 1));
	s4.addOutTran(new Transition(see, see, s5, 1));
	
	Transition tran2 = new Transition(pat, pat, s3, 1);
	tran2.getLabelOut().addRetTFString(mhp).addRetTFString(write).addRetTFString(rec.not());
	s5.addOutTran(tran2);
   ...
   public static void main(String[] args) {
      ...
      conflict s = new conflict();
      
      Utils.showDot(s.toDot("","LR"),"/home/javier/Dropbox/sharelatex/interoperability/conflict");
